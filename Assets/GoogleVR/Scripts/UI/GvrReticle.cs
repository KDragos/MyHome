...
public class GvrReticle : MonoBehaviour, IGvrGazePointer {
  ...
  private void SetUpTeleporter() {
    ...
    waypoints = new Transform[teleporter.transform.childCount];
    for (int i = 0; i < teleporter.transform.childCount; i++) {
        waypoints[i] = teleporter.transform.GetChild(i);
    }
  ...
  public void OnGazeTriggerEnd (Camera camera)
	{
		...
		if ((targetObj == null) || (!targetObj.tag.Equals ("Clickable"))) {
	        ...
	        cam.transform.position = Vector3.Lerp(cam.transform.position, destPos, 1);
		}
  }

  private void LocateActiveCamera ()
	{
		// Locate the active camera
